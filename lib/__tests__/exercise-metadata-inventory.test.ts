...
import {
  exerciseMetadataInventory,
  findInventoryItemsByNormalizedName,
  getInventoryTotals,
} from "@/lib/exercise-metadata-inventory";
...
describe("exercise metadata inventory", () => {
  it("covers every catalog exercise exactly once", () => {
    ...
    expect(
      new Set(exerciseMetadataInventory.map((item) => item.inventoryKey)).size,
    ).toBe(exerciseMetadataInventory.length);
    expect(
      exerciseMetadataInventory.map((item) => item.inventoryKey).sort(),
    ).toEqual(exerciseCatalog.map((item) => item.id).sort());
  });

  it("keeps registry metadata synchronized with the runtime catalog", () => {
    for (const catalogItem of exerciseCatalog) {
      const item = exerciseMetadataInventory.find(
        (entry) => entry.inventoryKey === catalogItem.id,
      );
  ...
  it("documents intentional completion reasons", () => {
    for (const item of exerciseMetadataInventory.filter(
      (entry) => entry.trackingType === "completion",
    )) {
  ...
  it("summarizes inventory totals by tracking type", () => {
    expect(getInventoryTotals()).toEqual({
      ...
      distance: expect.objectContaining({ names: expect.any(Number) }),
