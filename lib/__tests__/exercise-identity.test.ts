...
describe("exercise identity resolution", () => {
  ...
  it("resolves reviewed abbreviations only when they are explicitly approved", () => {
  ...
  it("keeps materially different equipment variants separate", () => {
    const bodyweight = resolveExerciseIdentityByReviewedName("Bodyweight squat");
    const box = resolveExerciseIdentityByReviewedName("Box squat");

    expect(bodyweight).toMatchObject({ status: "resolved", candidate: { canonicalId: "bodyweight-squat" } });
    expect(box).toMatchObject({ status: "resolved", candidate: { canonicalId: "box-squat" } });
  });

  it("leaves unreviewed custom names unresolved", () => {
    expect(resolveSystemExerciseIdentity({ displayName: "Custom rehab wall squat" })).toMatchObject({
      status: "unresolved"
    });
  });
});
