...
export type GeneratedPlanDraft = {
  ...
  phases: Array<{
    ...
    workouts: Array<{
      ...
      exercises: ProviderExerciseCandidate[];
    }>;
  }>;
};
...
export type MatchProvenance =
  ...
  | {
      kind: "invalid_id_name_match";
      invalidCatalogId: string;
      catalogId: string;
      via: "canonical_name" | "reviewed_alias";
    }
  | { kind: "custom_candidate"; normalizedKey: string }
  | { kind: "ambiguous"; normalizedKey: string; catalogIds: string[] }
  | {
      kind: "id_name_conflict";
      suppliedCatalogId: string;
      suppliedName: string;
      resolvedNameCatalogId: string;
    };

export type ExerciseResolutionIssue =
  | {
      code: "ambiguous_catalog_identity";
      message: string;
      candidates: Array<{ catalogId: string; name: string }>;
    }
  | {
      code: "supplied_id_name_conflict";
      message: string;
      suppliedCatalogId: string;
      resolvedNameCatalogId: string;
    }
...
export type NormalizedGeneratedExercise =
  ...
  | {
      status: "needs_review";
      displayName: string;
      prescription: NonNullable<ProviderExerciseCandidate["prescription"]>;
      issues: ExerciseResolutionIssue[];
      provenance: MatchProvenance;
      proposedCatalogExercises: Array<{ catalogId: string; name: string }>;
      candidate: ProviderExerciseCandidate;
    };

export type FatalGeneratedDraftError = {
  code:
    | "invalid_plan_hierarchy"
    | "invalid_exercise_structure"
    | "missing_required_prescription"
    | "unsupported_tracking_metadata";
  message: string;
  path: string;
};

export type NormalizedGeneratedPlanDraft = {
  plan: StructuredPlanInput;
  exercises: NormalizedGeneratedExercise[];
  reviewBlockingIssues: ExerciseResolutionIssue[];
};
...
const unilateralModes = new Set<UnilateralMode>([
  "bilateral",
  "same_each_side",
  "independent_sides",
]);
...
function normalizePrescription(candidate: ProviderExerciseCandidate) {
  ...
  if (!p || !Number.isInteger(p.sets) || p.sets <= 0 || !reps || !rest)
    return null;
...
function hasIncompatibleUnits(candidate: ProviderExerciseCandidate) {
  if (candidate.trackingType === "completion")
    return Boolean(
      candidate.loadUnit ||
      candidate.distanceUnit ||
      candidate.supportedLoadUnits?.length ||
      candidate.supportedDistanceUnits?.length,
    );
  if (candidate.trackingType === "weight_reps")
    return (
      !candidate.loadUnit ||
      !(candidate.supportedLoadUnits ?? []).includes(candidate.loadUnit)
    );
  if (
    candidate.trackingType === "distance" ||
    candidate.trackingType === "distance_duration"
  )
    return (
      !candidate.distanceUnit ||
      !(candidate.supportedDistanceUnits ?? []).includes(candidate.distanceUnit)
    );
...
export function validateReviewedAliasIntegrity(
  aliases = reviewedSystemAliases,
) {
  ...
  for (const exercise of exerciseCatalog) {
    ...
    if (!key)
      throw new Error(
        `Blank normalized canonical exercise name: ${exercise.id}`,
      );
    const existing = byKey.get(key);
    if (existing && existing !== exercise.id)
      throw new Error(`Canonical exercise name collision for ${exercise.name}`);
    byKey.set(key, exercise.id);
  }
  for (const [catalogId, values] of Object.entries(aliases)) {
    if (!getCatalogExercise(catalogId))
      throw new Error(`Reviewed alias target does not exist: ${catalogId}`);
    const targetKeys = new Set<string>();
    for (const alias of values) {
      const key = normalizeExerciseLookupKey(alias);
      if (!key)
        throw new Error(`Reviewed alias normalizes blank for ${catalogId}`);
      if (targetKeys.has(key))
        throw new Error(`Duplicate reviewed alias for ${catalogId}: ${alias}`);
      targetKeys.add(key);
      const existing = byKey.get(key);
      if (existing && existing !== catalogId)
        throw new Error(`Reviewed alias collision for ${alias}`);
...
function validateCustom(
  ...
  if (!normalizeOptionalString(candidate.name))
    issues.push({
      code: "invalid_custom_candidate",
      message: "Custom exercise name is required.",
      field: "name",
    });
  if (
    !normalizeOptionalString(candidate.coachingNote) &&
    !hasExerciseGuidance(guidance)
  )
    issues.push({
      code: "invalid_custom_candidate",
  ...
  if (!candidate.trackingType || !trackingTypes.has(candidate.trackingType))
    issues.push({
      code: "invalid_custom_candidate",
      message: "Supported tracking type is required.",
      field: "trackingType",
    });
  if (
    !candidate.unilateralMode ||
    !unilateralModes.has(candidate.unilateralMode)
  )
    issues.push({
      code: "invalid_custom_candidate",
      message: "Supported unilateral mode is required.",
      field: "unilateralMode",
    });
  if (hasIncompatibleUnits(candidate))
    issues.push({
      code: "invalid_custom_candidate",
      message: "Tracking units must match the tracking type.",
      field: "units",
    });
  if (!normalizeOptionalString(candidate.primaryValueLabel))
    issues.push({
      code: "invalid_custom_candidate",
      message: "Primary display label is required.",
      field: "primaryValueLabel",
    });
  if (
    requiresSecondaryLabel(candidate.trackingType) &&
    !normalizeOptionalString(candidate.secondaryValueLabel)
  )
    issues.push({
      code: "invalid_custom_candidate",
      message: "Secondary display label is required.",
...
export function resolveGeneratedExercise(
  candidate: ProviderExerciseCandidate,
): NormalizedGeneratedExercise {
  const prescription = normalizePrescription(candidate);
  if (!prescription)
    throw new Error(
      "resolveGeneratedExercise requires a valid prescription; call normalizeGeneratedPlanDraft for typed fatal errors.",
    );
  const candidateName = normalizeOptionalString(candidate.name);
  const key = normalizeExerciseLookupKey(candidateName);
  const byId =
    typeof candidate.proposedCatalogId === "string" &&
    candidate.proposedCatalogId
      ? resolveExerciseIdentityByCanonicalId(candidate.proposedCatalogId)
      : null;
  const byName = resolveExerciseIdentityByReviewedName(candidateName);
  if (byId?.status === "resolved") {
    if (
      byName.status === "resolved" &&
      byName.candidate.canonicalId !== byId.candidate.canonicalId
    ) {
      return {
        status: "needs_review",
        ...
        issues: [
          {
            code: "supplied_id_name_conflict",
            message: "Supplied catalog ID conflicts with exercise name.",
            suppliedCatalogId: byId.candidate.canonicalId,
            resolvedNameCatalogId: byName.candidate.canonicalId,
          },
        ],
        provenance: {
          kind: "id_name_conflict",
          suppliedCatalogId: byId.candidate.canonicalId,
          suppliedName: candidateName,
          resolvedNameCatalogId: byName.candidate.canonicalId,
        },
        proposedCatalogExercises: [byId.candidate, byName.candidate].map(
    ...
    return {
      status: "matched",
      exercise: toMatchedExercise(
        getCatalogExercise(byId.candidate.canonicalId)!,
        candidate,
        prescription,
      ),
      provenance: { kind: "catalog_id", catalogId: byId.candidate.canonicalId },
      planSpecificCoaching: normalizeOptionalString(candidate.coachingNote),
  ...
  if (byName.status === "ambiguous")
    return {
      ...
      issues: [
        {
          code: "ambiguous_catalog_identity",
          message: "Multiple catalog exercises match.",
          candidates: byName.candidates.map((c) => ({
            catalogId: c.canonicalId,
            name: c.displayName,
          })),
        },
      ],
      provenance: {
        kind: "ambiguous",
        normalizedKey: key,
        catalogIds: byName.candidates.map((c) => c.canonicalId),
      },
      proposedCatalogExercises: byName.candidates.map((c) => ({
        catalogId: c.canonicalId,
        name: c.displayName,
      })),
      candidate,
  ...
  if (byName.status === "resolved") {
    ...
    const provenance: MatchProvenance = candidate.proposedCatalogId
      ? {
          kind: "invalid_id_name_match",
          invalidCatalogId: candidate.proposedCatalogId,
          catalogId: item.id,
          via,
        }
      : via === "reviewed_alias"
        ? {
            kind: "reviewed_alias",
            catalogId: item.id,
            alias: byName.reviewedAlias!,
            normalizedKey: key,
          }
        : { kind: "canonical_name", catalogId: item.id, normalizedKey: key };
    return {
      status: "matched",
      exercise: toMatchedExercise(item, candidate, prescription),
      provenance,
      planSpecificCoaching: normalizeOptionalString(candidate.coachingNote),
      tempo: prescription.tempo,
      safetyNotes: normalizeOptionalNullableString(candidate.safetyNotes),
  ...
  if (GENERIC_AMBIGUOUS_NAMES.has(key)) {
    const proposed = exerciseCatalog
      .filter((e) => normalizeExerciseLookupKey(e.name).includes(key))
      .map((e) => ({ catalogId: e.id, name: e.name }));
    return {
      status: "needs_review",
      displayName: candidateName,
      prescription,
      issues: [
        {
          code: "ambiguous_catalog_identity",
          message: "Generic exercise name needs review.",
          candidates: proposed,
        },
      ],
      provenance: {
        kind: "ambiguous",
        normalizedKey: key,
        catalogIds: proposed.map((p) => p.catalogId),
      },
      proposedCatalogExercises: proposed,
      candidate,
    };
  }
  ...
  if (customIssues.length)
    return {
      ...
      prescription,
      issues: customIssues,
      provenance: { kind: "custom_candidate", normalizedKey: key },
      proposedCatalogExercises: [],
      candidate,
    };
  const normalizedVideoUrl =
    typeof candidate.videoUrl === "string"
...
function validateDraftStructure(draft: GeneratedPlanDraft) {
  ...
  if (draft.version !== "generated-plan-draft-v1")
    fatalErrors.push({
      code: "invalid_plan_hierarchy",
      message: "Unsupported draft version.",
      path: "version",
    });
  if (!normalizeOptionalString(draft.name))
    fatalErrors.push({
      code: "invalid_plan_hierarchy",
  ...
  if (
    ...
    draft.weeklySchedule.length === 0 ||
    !draft.weeklySchedule.every(isWeekday)
  )
    fatalErrors.push({
      code: "invalid_plan_hierarchy",
      message: "Draft needs a valid weekly schedule.",
      path: "weeklySchedule",
    });
  if (!Array.isArray(draft.phases) || draft.phases.length === 0)
    fatalErrors.push({
      code: "invalid_plan_hierarchy",
      message: "Draft needs at least one phase.",
      path: "phases",
    });
  (draft.phases ?? []).forEach((phase, pi) => {
    if (!normalizeOptionalString(phase?.goal))
      fatalErrors.push({
        code: "invalid_plan_hierarchy",
        message: "Phase goal is required.",
        path: `phases.${pi}.goal`,
      });
    if (!Array.isArray(phase.workouts) || phase.workouts.length === 0)
      fatalErrors.push({
        code: "invalid_plan_hierarchy",
        message: "Phase needs at least one workout.",
        path: `phases.${pi}.workouts`,
      });
    (phase.workouts ?? []).forEach((workout, wi) => {
      if (!normalizeOptionalString(workout?.name))
        fatalErrors.push({
          code: "invalid_plan_hierarchy",
          message: "Workout name is required.",
          path: `phases.${pi}.workouts.${wi}.name`,
        });
      if (
        ...
        fatalErrors.push({
          code: "invalid_plan_hierarchy",
          message: "Workout scheduled days are required.",
          path: `phases.${pi}.workouts.${wi}.scheduledDays`,
        });
      if (!Array.isArray(workout.exercises) || workout.exercises.length === 0)
        fatalErrors.push({
          code: "invalid_plan_hierarchy",
          message: "Workout needs at least one exercise.",
          path: `phases.${pi}.workouts.${wi}.exercises`,
        });
      (workout.exercises ?? []).forEach((exercise, ei) => {
        if (!exercise || typeof exercise.name !== "string")
          fatalErrors.push({
            code: "invalid_exercise_structure",
            message: "Exercise name is required.",
            path: `phases.${pi}.workouts.${wi}.exercises.${ei}.name`,
          });
        if (!normalizePrescription(exercise))
          fatalErrors.push({
            code: "missing_required_prescription",
            message:
              "Exercise prescription is required and must include positive integer sets, reps, and rest.",
...
export function normalizeGeneratedPlanDraft(
  ...
  if (!isStructuredPlanInput(plan))
    return {
      fatalErrors: [
        {
          code: "invalid_plan_hierarchy",
          message:
            "Generated draft does not satisfy the structured plan contract.",
          path: "plan",
        },
      ],
    };
  return {
    plan,
    exercises: resolved,
    reviewBlockingIssues: resolved.flatMap((e) =>
      e.status === "needs_review" ? e.issues : [],
    ),
  };
}

export function canSaveNormalizedGeneratedDraft(
  normalized: NormalizedGeneratedPlanDraft,
) {
  return (
    normalized.reviewBlockingIssues.length === 0 &&
    isStructuredPlanInput(normalized.plan)
  );
}
