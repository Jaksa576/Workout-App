              ...
              {supportsSetLogging ? (
                <div className="mt-2 overflow-hidden rounded-[18px] border border-border bg-surface-soft">
                  ...
                  {rows.map((row, rowIndex) => {
                    ...
                    return (
                      <div key={row.setId} className={`grid items-center gap-2 border-t border-border px-2 py-1.5 ${exercise.trackingType === "weight_reps" ? "grid-cols-[2.1rem_1fr_4.7rem_3.8rem_3rem]" : exercise.trackingType === "distance_duration" ? "grid-cols-[2.1rem_1fr_4rem_4.2rem_3rem]" : "grid-cols-[2.1rem_1fr_4rem_3rem]"} ${row.status === "completed" ? "bg-success/5" : ""}`}>
                        ...
                        {needsLoad ? (
                          <input aria-label={`Weight for set ${rowIndex + 1} of ${exercise.name}`} inputMode="decimal" className="min-w-0 rounded-xl border border-border bg-surface px-2 py-2 text-sm font-semibold" value={row.actualLoad ?? ""} onChange={(event) => { const value = event.target.value; const parsed = Number(value); if (value === "") updateSetResult(applyMetricSetEdit(row, { actualLoad: null })); else if (!Number.isNaN(parsed) && parsed >= 0) updateSetResult(applyMetricSetEdit(row, { actualLoad: parsed })); }} />
                        ) : null}
                        {exercise.trackingType === "duration" || exercise.trackingType === "distance_duration" ? (
                          <input aria-label={`${exercise.unilateralMode === "same_each_side" ? "Duration each side" : "Duration"} for set ${rowIndex + 1} of ${exercise.name}`} inputMode="numeric" className="min-w-0 rounded-xl border border-border bg-surface px-2 py-2 text-sm font-semibold" value={formatDurationInput(row.actualDurationSeconds)} placeholder="0:45" onChange={(event) => { const parsed = parseDurationInput(event.target.value); if (parsed === null) updateSetResult(applyMetricSetEdit(row, { actualDurationSeconds: null })); else if (!Number.isNaN(parsed) && parsed >= 0) updateSetResult(applyMetricSetEdit(row, { actualDurationSeconds: parsed })); }} />
                        ) : (
                          <input aria-label={`${exercise.unilateralMode === "same_each_side" ? "Reps each side" : "Reps"} for set ${rowIndex + 1} of ${exercise.name}`} inputMode="numeric" className="min-w-0 rounded-xl border border-border bg-surface px-2 py-2 text-sm font-semibold" value={row.actualReps ?? ""} onChange={(event) => { const value = event.target.value; const parsed = Number(value); if (value === "") updateSetResult(applyMetricSetEdit(row, { actualReps: null })); else if (Number.isInteger(parsed) && parsed >= 0) updateSetResult(applyMetricSetEdit(row, { actualReps: parsed })); }} />
                        )}
                        {exercise.trackingType === "distance_duration" ? (
                          <input aria-label={`Distance in ${exercise.distanceUnit ?? "mi"} for set ${rowIndex + 1} of ${exercise.name}`} inputMode="decimal" className="min-w-0 rounded-xl border border-border bg-surface px-2 py-2 text-sm font-semibold" value={row.actualDistance ?? ""} onChange={(event) => { const value = event.target.value; const parsed = Number(value); if (value === "") updateSetResult(applyMetricSetEdit(row, { actualDistance: null })); else if (!Number.isNaN(parsed) && parsed >= 0) updateSetResult(applyMetricSetEdit(row, { actualDistance: parsed })); }} />
                        ) : null}
