-- Issue #13 optional completed metrics.
...
begin
  ...
  if parent_tracking is null then raise exception 'exercise_set_results requires parent exercise_result'; end if;
  if new.set_order < 0 then raise exception 'set_order must be nonnegative'; end if;
  if new.set_kind = 'prescribed' and new.prescribed_set_index is null then raise exception 'prescribed set requires prescribed_set_index'; end if;
  if new.set_kind = 'added' and new.prescribed_set_index is not null then raise exception 'added set cannot have prescribed_set_index'; end if;
  ...
  if parent_tracking <> 'duration' and (new.actual_duration_seconds is not null or new.actual_left_duration_seconds is not null or new.actual_right_duration_seconds is not null) then raise exception 'duration values are not valid for this tracking type'; end if;
  if parent_tracking <> 'distance_duration' and new.actual_distance is not null then raise exception 'distance values are not valid for this tracking type'; end if;
  if new.status = 'completed' then
    if new.completed_at is null then raise exception 'completed set requires completed_at'; end if;
  else
    if new.completed_at is not null then raise exception 'only completed sets can have completed_at'; end if;
  end if;
  return new;
end;
$$;

-- Recreate finalize_workout_session with optional completed metric values.
