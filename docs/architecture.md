...
Generated exercises resolve through one deterministic matcher. Resolution order is: resolve a supplied exact stable catalog ID first; independently resolve the supplied name where possible; block true ID/name conflicts for review; accept a valid ID when the name is generic, unmatched, or an alias of the same exercise; fall back from invalid IDs to exact canonical-name and reviewed-alias matching; then apply generic-name ambiguity; and only finally allow validated custom fallback. The matcher uses trim/case/whitespace/punctuation normalization only; material exercise terms such as equipment, stance, loading, body position, unilateral variation, and movement variation are preserved unless an explicit reviewed alias covers them. Generic names that could refer to multiple catalog movements remain review-blocking instead of being assigned arbitrarily.
...
or removed. Generation, failure, back, cancel, and navigation before explicit save
