...
#### Issue #10 finalize RPC authorization boundary

`public.finalize_workout_session(jsonb, jsonb, jsonb)` intentionally remains the single write path for finalized workout execution rows. Later metric-entry work must extend this same RPC to persist prescribed scalar fields, actual scalar fields, and independent left/right fields; it must not add a second finalize route or parallel persistence system. Because the RPC is `SECURITY DEFINER`, it treats `auth.uid()` as the only trusted owner signal and validates every caller-supplied child ID inside the function: exercise entries must belong to the selected workout template, `source_workout_template_id` must match that template, duplicate exercise-result and exercise-entry IDs are rejected, and set rows must point only at exercise-result IDs inserted by the same RPC invocation for the newly saved session.
...
Inline active rows distinguish planned prescription, deterministic previous performance text, and current actual inputs. `weight_reps` rows collect optional decimal nonnegative load plus optional whole-number nonnegative reps; `reps_only` rows collect optional reps only. Blank values may be completed and must persist as `null`; malformed supplied values are rejected without inventing zero. Prescribed rows are generated from the workout prescription, cannot be removed, and user-added rows append after them.
...
### Issue #13 optional metric completion and defaulting
...
### PWA install and service-worker foundation

The app owns install guidance rather than relying on a browser-native first-visit prompt. The root shell may show a compact, dismissible install surface only when the browser exposes an install prompt or when iOS Safari needs Add to Home Screen instructions. The install surface is suppressed in standalone display mode, after dismissal cooldown, and on focused active-workout routes so it does not compete with workout controls.

The service worker is a conservative production-only foundation. `/sw.js` is registered client-side, served with an update-safe no-store cache policy, and currently avoids runtime caching by forwarding GET requests to the network. It must not cache authenticated HTML, Supabase/API traffic, session save requests, progression data, or other user-specific responses unless a future issue defines a safe allowlist and QA plan.

### Rest timer preference ownership

The active-workout rest timer remains the single timer authority. Global timer preference ownership is intentionally narrow: `profiles.default_rest_seconds` stores the user's default fallback rest duration and defaults to 90 seconds. Rest auto-start and timer-complete sound are application defaults that start enabled, while per-workout auto-start, sound, and explicit rest-override state live only in the existing active-workout draft and are cleared by the same successful-save or deliberate-discard lifecycle as other draft state. The draft stores both `workoutRestOverrideEnabled` and the selected `workoutDefaultRestSeconds` so disabling and re-enabling the override during a workout preserves the last selected duration until Reset to defaults is chosen.

Rest starts resolve duration from explicit active-draft override intent. When `workoutRestOverrideEnabled` is true, `workoutDefaultRestSeconds` applies to every exercise in the current workout and falls back only to the application timer fallback if invalid. When `workoutRestOverrideEnabled` is false, any stored workout duration is ignored and resolution uses exercise-level prescribed rest text, then global profile default rest, then the application timer fallback. Workout overrides do not rewrite exercise prescriptions or templates. Changing workout settings affects subsequent timer events only; an already running timer keeps its current remaining duration, while disabling sound before completion prevents the completion sound for that timer.

### Rest timer completion feedback lifecycle

Rest timer completion feedback belongs to the existing active-workout rest timer authority rather than a notification platform or background service. A natural transition from `running` to `expired` is the only eligible feedback point. The completion lifecycle identifier is derived from the timer's `startedAt`, `endsAt`, and completed-set/manual source, which prevents re-renders or recovered already-expired drafts from replaying feedback. Skip, clear, Finish, Discard, pause, and settings changes clear or bypass pending feedback; extending an expired timer starts a new running lifecycle with a new eligible completion identifier.

Vibration is independent of timer-complete sound and degrades silently when unsupported. Web Audio is an optional foreground cue: the workout flow may create/resume a reusable audio context after deliberate user gestures such as completing a set, manually starting rest, or enabling sound, then reuse that context for a short non-looping completion cue. The context is closed when rest-timer feedback is cleared by active-workout cleanup or when the workout flow unmounts.

## Canonical Exercise Identity And Alias Foundation (Issue #40)

The durable identity layer is additive to the existing plan/session model. The current reusable catalog remains the static TypeScript `exerciseCatalog`, while `exercise_entries.source_exercise_id` remains the legacy catalog-backed text key for compatibility. The new canonical identity foundation adds `exercise_identities`, `exercise_aliases`, and nullable `canonical_exercise_id` snapshots on `exercise_entries` and `exercise_results`; it does not rename compatibility tables, rewrite history, or force-merge custom exercises.

A canonical exercise identity represents one stable exercise concept with a canonical display name, normalized lookup key, ownership scope, optional owner user, equipment and movement qualifiers, active/superseded state, and JSON metadata extension points. The normalized lookup key is only for deterministic lookup and audits; it is not the identity. Reviewed aliases map approved naming variants to an identity within the correct ownership scope.

Resolution order is deterministic: exact canonical ID first, then exact reviewed alias/name lookup in scope, then unresolved when no single valid candidate exists. Runtime fuzzy matching and LLM-required lookup remain disallowed. LLM-assisted drafting may suggest candidates, but save paths must validate deterministically and preserve user review.

Saved plans and history continue to preserve display-name snapshots. New plan save validation resolves reviewed system aliases to `source_exercise_id`/canonical identity when a single approved match exists, while unknown or custom names remain usable and unresolved instead of being silently converted. Material variants such as bodyweight squat versus box squat remain separate identities through equipment, movement, and qualifier metadata.
...
## Generated plan draft boundary

Issue #62 establishes `GeneratedPlanDraft` as the provider-neutral application contract for future generated plan adapters. Provider-specific response shapes must map into this contract before entering core application code. Normalization is in-memory only: it produces a `StructuredPlanInput` compatible with the existing plan review/editor and must not call the structured plan save APIs or write plan, phase, workout, or exercise rows.

Generated exercises resolve through one deterministic matcher. Resolution order is exact stable catalog ID, exact conservatively normalized canonical catalog name, then exact reviewed alias. The matcher uses trim/case/whitespace/punctuation normalization only; material exercise terms such as equipment, stance, loading, body position, unilateral variation, and movement variation are preserved unless an explicit reviewed alias covers them. Generic names that could refer to multiple catalog movements remain review-blocking instead of being assigned arbitrarily.

A canonical exercise may own multiple reviewed aliases, but every normalized alias key must resolve to exactly one canonical exercise. Alias target drift or collisions are catalog-integrity failures and should be covered by tests before provider integration.

Resolution outcomes are `matched`, `custom`, and `needs_review`. Matched exercises use catalog-owned identity, canonical name, tracking type, unilateral mode, units, labels, reusable guidance, safety/caution guidance, and reviewed YouTube URL. Generated output may retain plan-specific prescriptions, rest, progression/coaching instructions, and user-specific notes, but it must not overwrite catalog-owned safety or media metadata. Custom exercises are allowed only when the provider supplies complete validated prescription, tracking metadata, labels, guidance/coaching, supported YouTube URL, and precise video search query. Ambiguous or conflicting identity becomes an exercise-level `needs_review` outcome that can enter review but blocks save until resolved.

The existing structured save boundary remains `/api/plans` or `/api/plans/[planId]` accepting `StructuredPlanInput`/`StructuredPlanSaveInput`, with persistence performed by `createStructuredPlanForUser` or `updateStructuredPlanForUser`. Generated normalization and review conversion intentionally stop before that boundary.
