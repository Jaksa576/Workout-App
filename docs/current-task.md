...
UI recovery now keeps stale drafts on the idle recovery decision surface until the user deliberately resumes or discards them, while preserving checked exercises and check-in data for an explicit resume. Malformed recovery data no longer points at an unavailable discard action; it exposes `Clear recovery data` for the current user's versioned storage key. After a successful save, `Start another workout` clears saved/draft/form state and returns to idle so the next session starts only after the user explicitly starts a new draft.
