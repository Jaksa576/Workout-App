...
Completion retries reuse the draft ID as the final session ID where practical. The local draft remains until `finalize_workout_session` succeeds; failed saves keep the draft and do not run progression.
