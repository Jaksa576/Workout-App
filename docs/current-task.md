...
### PR #35 follow-up — Issue #14 exercise-entry metadata backfill

The next Issue #14 patch addresses stale active `exercise_entries` metadata rather than changing the active-workout UI. Readiness findings: the active checklist renders set rows solely from stored `tracking_type`; `exercise_entries` is the durable plan/workout metadata snapshot; updating active entries does not rewrite completed `exercise_results` history snapshots; the backfill is deterministic through reviewed source IDs plus exact normalized name/prescription matches; and no runtime exercise-name inference is added.

The Supabase handoff migration is `supabase/migrations/20260712143000_issue14_backfill_exercise_entry_metadata.sql`. It updates only fallback rows still configured as `tracking_type = 'completion'`, `unilateral_mode = 'bilateral'`, and null units, leaving already-correct metadata untouched for idempotent reruns. It covers catalog-backed strength/reps/timed/distance-duration defaults and the QA-reported exact matches: Hamstring Bridge Hold, Lateral Band Walk, Half-Kneeling Hip Flexor Stretch, and Dead Bug. Ambiguous distance-only prescriptions remain completion rows for product-owner review rather than being forced into `distance_duration`.

Validation focus: run `supabase/verification/issue-14-exercise-entry-metadata-backfill-readonly.sql` before and after the migration to capture counts by tracking type and unilateral mode, remaining completion rows, numeric/timed/per-side/distance-looking completion rows, unit consistency, side-mode consistency, and explicit QA checks for the four reported exercises. Codex did not apply any hosted Supabase migration.
