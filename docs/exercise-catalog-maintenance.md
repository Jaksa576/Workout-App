...
- Historical migrations are immutable catalog history. Domain-review patches should add a new idempotent migration and read-only verification artifact that reconcile only system-owned identities/aliases and avoid plan, workout, session, result, and user-owned identity writes.
